...
public class Control extends JFrame {
	...
	public static JButton forward, reverse, leftTurn, rightTurn, stop, speedUp,
			slowDown;
	...
	private static class ButtonHandler implements ActionListener,
		...
		public void actionPerformed(ActionEvent ae) {
			...
			try {
				if (ae.getSource() == speedUp) {
					outData.writeInt(6);
				}
				if (ae.getSource() == slowDown) {
					outData.writeInt(7);
				}
				outData.flush(); // This forces any buffered output bytes to be
		...
		public void mouseReleased(MouseEvent moe) {
			try {
				if (moe.getSource() == forward || moe.getSource() == reverse
	...
	public static void connect() {
		try {
			...
			e.printStackTrace();
			throw new RuntimeException("NXT Connection failed");
		}
